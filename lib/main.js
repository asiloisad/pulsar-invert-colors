...
module.exports = {
  ...
  workspaceUpdate(state) {
    ...
    workspace.classList.toggle("invert-colors-workspace", state);
  ...
  editorUpdate(state) {
    const workspace = atom.views.getView(atom.workspace);
    workspace.classList.toggle("invert-colors-editor", state);
  ...
  imageUpdate(state) {
    const workspace = atom.views.getView(atom.workspace);
    workspace.classList.toggle("invert-colors-image", state);
    if (this.icons.image) {
  ...
  activateTile(key) {
    ...
    this.tooltips[key] = atom.tooltips.add(element, {
      title: () => `Invert ${key} colors is ${atom.config.get(`invert-colors.${key}State`) ? 'enabled' : 'disabled'}`,
